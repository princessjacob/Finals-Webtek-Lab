...
def appointment(request):
    tran = Transaction.objects.all()[:1]
...
def editrequest(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return HttpResponseRedirect('/editrequest/')
    else:
        form = Request.objects.get(pk=1)
        update_form = UpdateForm(instance=req)

    return render(request, 'custModule/editrequest.html', {'form': form, 'req':req})
